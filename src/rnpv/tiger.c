...
static void
initcontext(RNPV_TIGER_CTX *ctx, uint8_t pad)
{
...
static int
setversion(RNPV_TIGER_CTX *ctx, int version)
{
	switch(version) {
	case 0:
	case 1:
...
void
rnpv_TIGER_Init(RNPV_TIGER_CTX *ctx)
{
	if (ctx) {
		initcontext(ctx, 0x01);
	}
...
void
rnpv_TIGER2_Init(RNPV_TIGER_CTX *ctx)
{
	if (ctx) {
		initcontext(ctx, 0x80);
	}
...
void
rnpv_TIGER_Update(RNPV_TIGER_CTX *ctx, const void *data, size_t length)
{
...
void
rnpv_TIGER_Final(uint8_t *digest, RNPV_TIGER_CTX *ctx)
{
	uint64_t	le[3];
	int		indian = 1;
	int		i;
...
char *
rnpv_TIGER_End(RNPV_TIGER_CTX *ctx, char *buf)
{   
	int	i;
...
char *
rnpv_TIGER_File(char *filename, char *buf, int version)
{   
	RNPV_TIGER_CTX	ctx;
	...
	while ((num = read(fd, buffer, sizeof(buffer))) > 0) {
		rnpv_TIGER_Update(&ctx, buffer, (size_t)num);
	}
	oerrno = errno;
	close(fd);
	errno = oerrno;
	return (num < 0) ? NULL : rnpv_TIGER_End(&ctx, buf);
}

char *
rnpv_TIGER_Data(const uint8_t *data, size_t len, char *buf, int version)
{   
	RNPV_TIGER_CTX	ctx;
	...
	rnpv_TIGER_Update(&ctx, data, len);
	return rnpv_TIGER_End(&ctx, buf);
}
