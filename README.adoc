...
== link:docs/crypto-backends.adoc[Crypto backends]

librnp can be built against several crypto backends. The choice of backend
determines which algorithms are available and on which platforms librnp runs
without an additional crypto dependency. See link:docs/crypto-backends.adoc[Crypto backends]
for the full matrix (OpenSSL 1.1.x, OpenSSL 3.x, LibreSSL, Botan) and the
feature tradeoffs of each (PQC, AEAD-OCB, Brainpool, etc.).
