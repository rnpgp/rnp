...
cmake -DCMAKE_INSTALL_PREFIX=/usr -DBUILD_SHARED_LIBS=on \
  -DBUILD_TESTING=off -DDOWNLOAD_SEXP=on ../rnp/
...
cmake -DCMAKE_INSTALL_PREFIX=/usr/local -DBUILD_SHARED_LIBS=on \
  -DBUILD_TESTING=off -DDOWNLOAD_SEXP=on .
...
# Run CMake
cmake -DBUILD_SHARED_LIBS=yes -G "MSYS Makefiles" -DDOWNLOAD_SEXP=on -DBUILD_TESTING=off ../rnp
...
mkdir rnp-build
cd rnp-build
cmake -B . -G "Visual Studio 16 2019" -A x64 -DCMAKE_TOOLCHAIN_FILE=%VCPKG_ROOT%\scripts\buildsystems\vcpkg.cmake -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=off ../rnp -DDOWNLOAD_SEXP=on
...
== Compile from source without access to github.com

Download the latest release from https://github.com/rnpgp/sexp and expand it
Enter the source folder and run the following commands:
...
mkdir build
cd build
cmake -DCMAKE_INSTALL_PREFIX=/usr/local -DWITH_SEXP_TESTS=off -DWITH_SEXP_CLI=off ..
cmake --build build
cmake --install build
----

Download a release from https://github.com/rnpgp/rnp and expand it
Enter the source folder and run the following commands:
...
cd build
cmake -DCMAKE_INSTALL_PREFIX=/usr/local -DBUILD_SHARED_LIBS=on \
  -DBUILD_TESTING=off -DDOWNLOAD_SEXP=off ..
cmake --build build
cmake --install build
----
