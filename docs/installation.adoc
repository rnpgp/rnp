...
RNP ebuilds are available in overlay repository named `rnp`.

=== Using layman (the old way)

Prerequisite: make sure `layman` is installed on your system.
...
=== Using eselect-repository (the new way)

Prerequisite: make sure `eselect-repository` is installed on your system.
