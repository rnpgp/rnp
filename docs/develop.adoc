...
Downstream consumption of the installed library (CMake `find_package`, pkg-config, raw
compiler flags, against both shared and static builds) is smoke-tested by
`ci/tests/downstream-consumer.sh`; see link:packaging.adoc[Consuming librnp in downstream projects]
for the corresponding consumer documentation.
