...
# Try to read PACKAGE_VERSION from source and take it as the RPM version
# number.

readonly __file_dir=$(dirname $0)
readonly version_file=${__file_dir}/../../../src/lib/config.h

if [ -r ${version_file} ]; then
  readonly default_version=$( \
    /usr/bin/env grep define.*PACKAGE_VERSION ${version_file} | \
    awk '{print $3}' | \
    sed 's/"//g' \
  )
fi
